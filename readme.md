...
| GET    | `/api/v1/farms/{farmID}/weather-alerts/{id}`    | Yes  | Get a specific weather alert  |
| PUT    | `/api/v1/farms/{farmID}/weather-alerts/{id}`    | Yes  | Update a weather alert        |
| DELETE | `/api/v1/farms/{farmID}/weather-alerts/{id}`    | Yes  | Delete a weather alert        |
...
Classification predicts discrete categories like "rice" or "wheat". Regression predicts continuous numbers like 45.7 or 123.4. Since we're predicting _which crop_, not _how much of something_, classification is the correct approach.
...
# Redis (use REDIS_URL for cloud providers, or REDIS_ADDR for local)
...
psql -d farm_fusion -f migrations/001_create_users_table.up.sql
psql -d farm_fusion -f migrations/002_create_refresh_tokens_table.up.sql
psql -d farm_fusion -f migrations/003_create_farms_table.up.sql
psql -d farm_fusion -f migrations/004_create_weather_alerts_table.up.sql
psql -d farm_fusion -f migrations/005_add_location_key_to_farms.up.sql
psql -d farm_fusion -f migrations/006_create_notification_log_table.up.sql
```
...
```bash
# Terminal 1 — Scheduler (runs daily at 5 AM, or set SCHEDULER_RUN_ONCE=true to run immediately)
go build -o bin/scheduler cmd/scheduler/main.go
./bin/scheduler

# Terminal 2 — Worker (consumes RabbitMQ queue and sends emails)
